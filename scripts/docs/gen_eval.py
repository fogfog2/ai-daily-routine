...
BODY = r"""
  ...
  <section>
    ...
    <ul>
      ...
      <li><strong>강건성 평가.</strong> 정확도만이 아니라 <a href="adversarial-robustness.html">최악의 경우</a>를 함께 잰다.</li>
    ...
    <div class="note">
      <b>지표를 고르는 일이 곧 개선 방향을 고르는 일이다.</b>
      <a href="object-tracking.html">다중 객체 추적</a>이 알기 쉬운 사례다 —
      오래 쓰인 MOTA 는 검출 오류와 정체성 오류를 한 분수에 더해서,
      정체성 실패가 몇 배로 늘어도 점수가 거의 안 움직인다.
      그래서 MOTA 만 보면 <em>검출만 잘하는 추적기</em>가 이긴다.
      뒤에 나온 HOTA 는 둘을 <strong>따로 재서 기하평균으로 합쳐</strong> 이 편향을 없앴다.
      <em>총점보다 분해가 유용하다</em>는 것이 여기서 얻은 교훈이다.
    </div>
    <p>
