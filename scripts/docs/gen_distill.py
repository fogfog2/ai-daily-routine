...
BODY = r"""
  ...
  <section>
    ...
    <p>
      더 이상한 갈래도 있다. <a href="dino-self-distillation.html">자기증류</a>는
      <strong>교사를 아예 두지 않는다</strong> — 교사가 학생 가중치의 이동평균이다.
      옮길 지식이 바깥에 없는데도 학습이 되는데, 그러려면 붕괴를 막는 장치가 따로 필요하다.
      온도가 여기서는 "부드러운 정답"을 만드는 손잡이가 아니라 <em>그 장치의 한쪽</em>이 된다.
    </p>
    <p>
