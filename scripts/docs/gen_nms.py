...
BODY = r"""
  ...
  <section>
    ...
    <p>
      낮은 점수를 <em>버리지 않고 뒤로 넘긴다</em>는 이 발상은
      <a href="object-tracking.html">다중 객체 추적</a>에서 한 번 더 나온다.
      가려진 대상의 상자는 점수가 낮게 나오므로,
      추적기가 그것을 <strong>기존 트랙의 예측 위치라는 맥락</strong>과 함께 다시 보면
      오검출과 갈라진다 — 같은 정보를 두 번 쓰는 셈이다.
    </p>
  </section>
