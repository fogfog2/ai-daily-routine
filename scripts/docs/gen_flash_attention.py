...
BODY = r"""
  ...
  <section>
    ...
    <p>
      다만 사라진 것은 <em>메모리 상한</em>이지 비용 전체가 아니다.
      연산은 여전히 <code>O(N²)</code>이고, 70B급에서 128K 를 넣으면
      어텐션이 프리필 연산의 7할을 먹는다.
      거기에 KV 캐시 용량과 위치 인코딩 확장이라는 별개의 벽이 더 있다 —
      <a href="long-context.html">긴 문맥</a> 편에서 이 셋을 나눠 센다.
    </p>
    <p>
