...
DOCS = [
    ...
    ("CNN 기초", "합성곱·풀링·수용 영역",
     "", "cnn-basics", "basics",
     "이미지의 규칙성을 구조에 새긴 두 제약. 수용 영역과 해상도의 맞바꿈, 그리고 잔차 연결.", "2026-08-11",
     ["vision", "arch", "training"]),

    ("잔차 연결", "기본값을 항등으로 바꾸기",
     "", "residual-connections", "arch",
     "층을 쌓았더니 학습 오차가 올랐다. 한 줄의 덧셈이 그것을 뒤집었고, 그것만으로는 이번엔 터진다.",
     "2026-08-19", ["common", "arch"]),
    ...
    ("NAS", "구조를 자동으로 찾기",
     "", "nas", "arch",
     "평가가 비싸다는 난점과 가중치 공유. 목표에 실측 지연을 넣는 것이 진짜 기여였다.", "2026-08-11",
     ["vision", "on-device", "arch"]),
    ("경량 백본", "휴대폰에서 도는 신경망",
     "", "efficient-backbone", "arch",
     "공간과 채널을 분리해 8~9배를 아끼는 법, 그리고 FLOPs 를 줄여도 안 빨라지는 이유.", "2026-08-11",
     ["vision", "on-device", "arch"]),
    ...
    ("메트릭 러닝", "같은 것은 가깝게, 다른 것은 멀게",
     "", "metric-learning", "train",
     "클래스가 정해지지 않은 세계에서 거리 자체를 학습한다. 삼중항에서 각도 마진까지.", "2026-08-11",
    ...
    ("DINO 와 자기증류", "라벨 없이 배운 시각 특징",
     ...
     ["vision", "feature", "training"]),
    ("ControlNet", "말로 못 하는 것을 그림으로",
     "", "controlnet", "train",
     "텍스트로는 공간을 지정할 수 없다. 원본을 얼리고 0에서 시작하는 조건 통제.", "2026-08-11",
     ["vision", "generative", "enhance"]),
    ("Super-Resolution", "없는 픽셀을 지어내는 일",
     "", "super-resolution", "train",
     "픽셀 오차를 줄이면 여러 답의 평균이 되어 흐려진다. 목표를 바꾼 과정과 그 대가.", "2026-08-11",
     ["vision", "enhance", "generative"]),
    ("노이즈 제거", "무작위인 것을 가려내기",
     "", "denoising", "train",
     "노이즈와 세부가 둘 다 고주파라는 난점. 깨끗한 정답 없이 학습하는 법까지.", "2026-08-11",
     ["vision", "enhance", "training"]),
    ("디블러링", "겹친 빛을 되돌리기",
     "", "deblurring", "train",
     "흐림은 곱셈이 아니라 합성곱이다. 그냥 나누면 터지고, 지워진 주파수는 되살아나지 않는다.", "2026-08-20",
     ["vision", "enhance", "training"]),
    ...
    ("RLVR", "채점 가능한 것만 보상한다",
     "", "rlvr", "align",
     "보상을 신경망이 아니라 채점 프로그램이 준다. GRPO 가 가치망까지 걷어내는 대신 생기는 새 실패.", "2026-08-17", ["llm", "alignment", "training"]),
    ...
    ("긴 문맥", "넣을 수 있는 길이와 쓸 수 있는 길이",
     "", "long-context", "infer",
     "KV 캐시는 선형, 어텐션은 제곱 — 5만 토큰 언저리에서 병목이 뒤바뀐다. 그리고 표방 길이는 유효 길이가 아니다.", "2026-08-16",
     ["llm", "efficiency", "arch"]),
    ...
    ("온디바이스 양자화", "INT8 로 실제 굴리기",
     "", "ondevice-quantization", "infer",
     "가중치만이 아니라 활성값까지. per-channel 이 필요한 이유를 단계 수로 보인다.", "2026-08-11",
     ["vision", "on-device", "efficiency"]),
    ("캘리브레이션", "보지 못한 입력의 범위를 정하기",
     "", "calibration", "infer",
     "활성값 범위를 몇백 장으로 추정한다. 최댓값을 쓸 것인가 잘라낼 것인가.", "2026-08-11",
     ["vision", "on-device", "efficiency"]),
    ("QAT", "깎일 것을 알고 자라게 하기",
     "", "qat", "infer",
     "반올림은 미분 불가다. STE 로 우회해 양자화를 학습에 넣는 방법.", "2026-08-11",
     ["common", "on-device", "training"]),
    ...
    ("데이터 파이프라인", "모델보다 오래 걸리는 일",
     "", "data-pipeline", "infra",
     "수집·라벨링·검수는 일방향이 아니라 순환이다. 라벨 불일치를 활용하는 법.", "2026-08-11",
     ["common", "systems", "training"]),
    ("실험 관리", "재현할 수 없는 개선은 개선이 아니다",
     "", "experiment-tracking", "infra",
     "무엇을 바꿔서 좋아졌는지 모르는 상태를 막는 법. 완전 재현이 불가능한 이유까지.", "2026-08-11",
     ["common", "systems"]),
    ...
    ("Few-Shot / Zero-Shot", "예시가 거의 없을 때",
     "", "few-shot-learning", "apply",
     "분류를 비교로 바꾼다. 프로토타입과 텍스트 대표, 그리고 좋은 표현이 알고리즘보다 중요하다는 결과.", "2026-08-11",
     ["vision", "feature", "training"]),
    ("Vision-Language 모델", "비교에서 생성으로",
     "", "vision-language-models", "apply",
     "CLIP 이 못 하던 생성. 이미지를 토큰으로 옮기는 커넥터와 해상도·비용의 맞바꿈.", "2026-08-11",
     ["vision", "systems", "feature"]),
    ("벡터 검색", "임베딩을 실제로 쓸 수 있게",
     "", "vector-search", "apply",
     "전수 비교가 불가능해지는 지점과 근사 탐색 세 전략. 검색 파이프라인의 1단계.", "2026-08-11",
     ["common", "feature", "systems"]),
    ("모바일 추론 런타임", "모델과 기기 사이",
     "", "mobile-runtime", "apply",
     "연산자 지원이 모델을 정한다. 그래프 분할과 전처리 불일치, 그리고 발열.", "2026-08-11",
     ["vision", "on-device", "systems"]),
    ...
    ("유사도 임계값", "어디서 자를 것인가",
     "", "similarity-threshold", "eval",
     "두 분포는 반드시 겹친다. 규모가 커지면 같은 임계값이 무너지는 이유까지.", "2026-08-11",
     ["vision", "feature", "safety"]),
    ("화질 평가 지표", "무엇을 재고 있는가",
     "", "image-quality-metrics", "eval",
     "PSNR 이 같은데 체감이 다른 이유. 구조를 따로 재는 SSIM 과 사람 판단을 학습한 LPIPS.", "2026-08-11",
     ["vision", "enhance", "safety"]),
    ("환각", "왜 그럴듯하게 틀리는가",
     "", "hallucination", "eval",
     "구조에서 비롯된 원인과, 완화가 제거를 뜻하지 않는 이유.", "2026-08-10", ["llm", "safety"]),
    ("적대적 예제와 강건성", "보이지 않는 변화로 답이 뒤집힌다",
     "", "adversarial-robustness", "eval",
     "픽셀당 ±8로 정확도가 0이 된다. 버그가 아니라 데이터의 성질이라는 관점까지.", "2026-08-11",
    ...
    ("NeRF & 3D 표현", "공간을 무엇으로 담을 것인가",
     "", "nerf-3d", "detect",
     "메시가 담지 못하는 것을 함수로, 다시 명시적 덩어리로. 3DGS 이후의 분화까지.", "2026-08-11",
     ["vision", "arch", "generative"]),
    ("Optical Flow", "시간 축의 대응 문제",
     "", "optical-flow", "detect",
     "미지수 2개에 식 1개인 조리개 문제. RAFT의 상관 볼륨과 합성 데이터 의존.", "2026-08-11",
     ["vision", "detection", "feature"]),
    ("깊이 추정", "카메라가 버린 정보를 되찾기",
     "", "depth-estimation", "detect",
     "스테레오·능동센서·단안의 갈림. 오차가 거리의 제곱에 비례하는 이유와 스케일 모호성.", "2026-08-11",
     ["vision", "detection", "arch"]),
    ("ISP", "센서 값을 사진으로 만드는 일",
     "", "isp-pipeline", "detect",
     "화소당 색이 하나뿐이라 66%가 추정값이다. 사람 눈과 인식 모델의 목표가 어긋나는 지점.", "2026-08-11",
     ["vision", "enhance", "on-device"]),
    ("HDR 과 톤 매핑", "담을 수 없는 밝기를 다루는 법",
     "", "hdr-tone-mapping", "detect",
     "넓히는 문제와 좁히는 문제는 방향이 반대다. 휴대폰이 브라케팅 대신 짧은 노출 버스트를 고른 이유.", "2026-08-14",
     ["vision", "enhance", "on-device"]),
    ("다중 객체 추적", "정체성은 곱해진다",
     "", "object-tracking", "detect",
     "검출 오류는 그 프레임에서 끝나지만 정체성 오류는 그 뒤를 전부 오염시킨다.", "2026-08-13",
     ["vision", "detection", "feature"]),
    ("분할", "어디까지가 그것인가",
     "", "segmentation", "detect",
     "시맨틱·인스턴스·판옵틱의 구분, 해상도와 시야의 맞바꿈, 그리고 라벨 비용이라는 장벽.", "2026-08-11",
     ["vision", "detection", "arch"]),
    ("지역 특징과 매칭", "같은 지점을 다시 찾기",
     "", "local-features", "detect",
     "전역 벡터가 못 하는 점 대 점 대응. 반복 무늬라는 적과 비율 검사·RANSAC.", "2026-08-11",
     ["vision", "feature"]),
    ("재식별 (Re-ID)", "다른 카메라의 같은 사람",
     "", "person-reid", "detect",
     "뷰·조명·가림이 다 바뀌는데 같다고 해야 한다. 학습에 없던 사람을 찾는 검색 문제.", "2026-08-11",
     ["vision", "feature", "detection"]),
    ("IoU 계열 손실", "상자를 겹침으로 배우기",
     "", "iou-losses", "detect",
     "겹치지 않으면 IoU 의 기울기가 0 이 된다. 그 평지를 GIoU·DIoU·CIoU 가 메워 온 방식.", "2026-08-18",
     ["vision", "detection", "training"]),
    ("NMS", "중복 상자를 걷어내는 마지막 수작업",
     "", "nms", "detect",
     "학습에는 중복이 이롭고 출력에는 해롭다는 어긋남. 가림 상황의 실패와 NMS-free 흐름.", "2026-08-11",
     ["vision", "detection", "efficiency"]),
