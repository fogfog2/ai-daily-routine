...
### 관심사 — 온디바이스 양자화 `on-device` (2026-08-11 점검)
...
| 주제 | 단계 | 태그 | 왜 필요한가 | 상태 |
|---|---|---|---|---|
| **온디바이스 양자화 (INT8 실전)** | 05 추론 | `vision` `on-device` `efficiency` | per-tensor/per-channel 차이를 계산으로 보이고, 활성값 캘리브레이션·QAT·연산자 융합까지. 기존 양자화 문서(LLM 중심)와 겹치지 않는다 | **대기(최우선)** |
| **캘리브레이션과 활성값 범위** | 05 추론 | `vision` `on-device` | 가중치는 고정이지만 활성값 범위는 데이터로 추정해야 한다. 대표 데이터 선정이 정확도를 가른다 | 대기 |
| **QAT — 학습에 양자화를 넣기** | 05 추론 | `common` `on-device` `training` | 기존 문서에 QAT 가 2회 언급되고 끝난다. STE 로 미분 불가를 우회하는 방법 | 대기 |
